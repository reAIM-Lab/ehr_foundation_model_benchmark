...
from femr.omop_meds_tutorial.evaluation.generate_mtpp_features import get_motor_features_name
...
#         create_omop_meds_tutorial_arg_parser,
#     )
#     from femr.omop_meds_tutorial.evaluation.generate_mtpp_features import (  # type: ignore
...
'''
how to understand feature_times in compute_features() and prediction_time in join_labels
  1. compute_features() output:
    - feature_times: Actual patient event times where embeddings were extracted (≤ prediction_time)
    - These times may not equal prediction_time
  2. join_labels() output:
    - times: Same as feature_times above (actual event times)
    - prediction_times: NEW field - the exact prediction_time from the original labels
    - This is why after join_labels(), you have both the feature extraction time AND the label prediction time

  3. Why they differ: The model extracts features at the last available event ≤ prediction_time, which is typically not exactly at prediction_time
  (patients don't have events at arbitrary prediction times).

  4. number of feature_times and prediction_time
  prediction_time always equal to number of rows in labels.
  but features_time may be fewer since there may be multiple prediction rows equal to the same feature time.
'''