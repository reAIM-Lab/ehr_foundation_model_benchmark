...
def rotate_every_two_v2(x: torch.Tensor) -> torch.Tensor:
...
def fixed_pos_embedding(ages: torch.Tensor, dim: int, dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
...
def apply_rotary_pos_emb(x: torch.Tensor, sincos: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    sin, cos = sincos
...
# === Transformer (copied behavior from transformer.py; unchanged) ===
...
# === Task heads (copied behavior from transformer.py) ===
...
class FEMRModel(transformers.PreTrainedModel):
    ...
    def __init__(self, config: femr.models.config.FEMRModelConfig, **kwargs):
        ...
        if self.config.model_type == "mamba":
            ...
            hidden_size = self.config.mamba_config.hidden_size
            if hidden_size is None:
                hidden_size = self.backbone.out_norm.weight.shape[0]
        ...
        if self.config.task_config is not None:
            self.task_model = self.create_task_head(hidden_size)

    def create_task_head(self, hidden_size: int) -> nn.Module:
